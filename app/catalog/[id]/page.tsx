...
export default function ProductDetailPage({ params }: ProductPageProps) {
    ...
    async function fetchData() {
      try {
        ...
        if (prodData.success && prodData.products) {
          ...
          const found = prodData.products.find((p: Product) => p.id === id);
          if (found) {
            setProduct(found);
          }
        }
  ...
  if (!mounted || loading) {
    return (
      <>
        <Header />
        <div style={{ minHeight: "70vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#0a0a0a", color: "var(--gold)" }}>
          <p style={{ letterSpacing: "0.15em", textTransform: "uppercase", fontSize: "13px" }}>Loading Masterpiece Details...</p>
        </div>
        <Footer />
      </>
    );
  ...
  return (
    <>
      ...
      <main className="product-detail-page" suppressHydrationWarning style={{ background: "var(--ivory, #0a0a0a)", minHeight: "100vh", padding: "40px 0 80px 0" }}>
